...
import com.bugzero.rarego.boundedContext.auction.event.AuctionCreatedEvent;
import com.bugzero.rarego.boundedContext.auction.out.AuctionMemberRepository;
import com.bugzero.rarego.boundedContext.auction.out.AuctionOrderRepository;
import com.bugzero.rarego.boundedContext.auction.out.AuctionRepository;
import com.bugzero.rarego.boundedContext.auction.out.BidRepository;
import com.bugzero.rarego.boundedContext.product.domain.*;
...
public class AuctionDataInit implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        ProductMember seller = createOrGetProductMember(1L, "seller@test.com", "판매자_제로");
        ...
        // 1. [진행중] 입찰이 있는 경매
        Product product1 = createProduct(seller, "[1-1] 레고 밀레니엄 팔콘", 10_000);
        ...
        // 3-1. 종료 + 입찰 있음 (낙찰 대상)
        Product product4 = createProduct(seller, "테스트 상품 1 (낙찰대상)", 10_000);
        // 시작: 2분 전, 종료: 1분 전
        Auction auctionEndWithBid = createAuction(product4.getId(), product4.getSeller().getId(), -2, -1, 10_000);
        createBid(auctionEndWithBid, buyer, 15_000);
        createBid(auctionEndWithBid, buyer, 20_000); // 판매자가 입찰? 테스트라 가정
        createBid(auctionEndWithBid, buyer, 25_000);

        // 3-2. 종료 + 입찰 없음 (유찰 대상)
        Product product5 = createProduct(seller, "테스트 상품 2 (유찰대상)", 20_000);
        // 시작: 2분 전, 종료: 현재 (0분)
        createAuction(product5.getId(), product4.getSeller().getId(), -2, 0, 20_000);

        // 3-3. 종료 + 입찰 1건 (낙찰 대상)
        Product product6 = createProduct(seller, "테스트 상품 3 (단독입찰)", 30_000);
        // 시작: 3분 전, 종료: 현재 (0분)
        Auction auctionEndWithOneBid = createAuction(product6.getId(), product4.getSeller().getId(), -3, 0, 30_000);
        createBid(auctionEndWithOneBid, buyer, 35_000);

        // 3-4. 진행 중 + 1분 후 종료 (동적 스케줄링 테스트용)
        Product product7 = createProduct(seller, "테스트 상품 4 (1분후 종료)", 40_000);
        Auction auctionSoonEnd1 = createAuction(product7.getId(), product4.getSeller().getId(), -1, 1, 40_000);
        ...
        Auction auctionSoonEnd5 = createAuction(product8.getId(), product4.getSeller().getId(), -1, 5, 50_000);
        ...
        // 1. 낙찰될 상품 생성
        Product soldProduct = createProduct(seller, "[낙찰 완료] 레고 타이타닉", 500_000);
        ...
        Product withdrawProduct1 = createProduct(seller, "[포기 가능] 유찰된 레고", 100_000);
        ...
        Product withdrawProduct2 = createProduct(seller, "[포기 가능] 결제 실패 레고", 200_000);
        ...
        Product withdrawProduct3 = createProduct(seller, "[포기 불가] 결제 완료 레고", 300_000);
        ...
        Product withdrawProduct4 = createProduct(seller, "[포기 불가] 결제 대기 레고", 400_000);
        ...
        Product withdrawProduct5 = createProduct(seller, "[포기 불가] 진행 중 레고", 500_000);
    ...
    private ProductMember createOrGetProductMember(Long id, String email, String nickname) {
        return productMemberRepository.findById(id)
                .orElseGet(() -> productMemberRepository.save(
    ...
    private Product createProduct(ProductMember seller, String name, int startPrice) {
