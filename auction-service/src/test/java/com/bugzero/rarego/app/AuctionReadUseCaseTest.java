...
class AuctionReadUseCaseTest {
	...
	class GetMyBidsTest {
		...
		void getMyBids_success() {
			...
			given(bidRepository.findAllByBidderIdAndAuctionStatus(eq(memberId), isNull(), eq(pageable))).willReturn(
				bidPage);
		...
		void getMyBids_empty() {
			...
			given(support.getPublicMember(memberPublicId)).willReturn(member);
			given(bidRepository.findAllByBidderIdAndAuctionStatus(eq(10L), isNull(), eq(pageable))).willReturn(
				emptyPage);
		...
		void getMyBids_withStatusFilter() {
			...
			given(support.getPublicMember(memberPublicId)).willReturn(member);
			given(bidRepository.findAllByBidderIdAndAuctionStatus(eq(memberId), eq(AuctionStatus.IN_PROGRESS),
				eq(pageable))).willReturn(bidPage);
			given(auctionRepository.findAllById(Set.of(auctionId))).willReturn(List.of(auction));

			// when
			PagedResponseDto<MyBidResponseDto> result = auctionReadUseCase.getMyBids(memberPublicId,
				AuctionStatus.IN_PROGRESS, pageable);
	...
	class GetAuctionDetailTest {
		...
		void getAuctionDetail_member_withBid() {
			...
			given(bidRepository.findTopByAuctionIdAndBidderIdOrderByBidAmountDesc(auctionId, memberId)).willReturn(
				Optional.of(myLastBid));
		...
		void getAuctionDetail_seller() {
			...
			given(bidRepository.findTopByAuctionIdAndBidderIdOrderByBidAmountDesc(auctionId, sellerId)).willReturn(
				Optional.empty());
	...
	class GetAuctionsTest {
		...
		void getAuctions_success() {
			...
			given(auctionRepository.findAllBySearchConditions(any(), any(), any(), any(), any())).willReturn(
				auctionPage);
		...
		void getAuctions_with_search_condition() {
			...
			Page<Auction> auctionPage = new PageImpl<>(List.of(auction), pageable, 1);
			given(auctionRepository.findAllBySearchConditions(any(), any(), eq(List.of(productId)),
				eq(List.of(productId)), any())).willReturn(auctionPage);
	...
	class GetMyAuctionOrdersTest {
		...
		void getMyAuctionOrders_success() {
			...
			given(support.getPublicMember(memberPublicId)).willReturn(member);
			given(auctionOrderRepository.findAllByBidderIdAndStatus(eq(memberId), isNull(), any())).willReturn(
				orderPage);
	...
	class GetMyBookmarksTest {
		...
		void getMyBookmarks_success() {
			...
			// when
			PagedResponseDto<AuctionBookmarkListResponseDto> result = auctionReadUseCase.getMyBookmarks(publicId,
				pageable);
